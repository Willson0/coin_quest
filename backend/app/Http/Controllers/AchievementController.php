...
{
    public function pin (Achievements $achievement, Request $request) {
        ...
        if (count($pinned_achievements) >= 3) array_shift($pinned_achievements);
    ...
    public function unpin (Achievements $achievement, Request $request) {
        $user = User::where("telegram_id", $request["initData"]["user"]["id"])->firstOrFail();
